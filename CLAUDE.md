...
- **Screener radios (re-verified live 2026-09-05):** `<p>question</p>` followed by `<fieldset role="radiogroup">` of `div[role="radio"]` wrappers. Since ~2026-09-02 each wrapper's `aria-label` repeats the QUESTION (or the group label: "Gender", "Race/Ethnicity", "Veteran status"; the CC-305 disability group uses `aria-labelledby`), and the option text ("Yes"/"No"/"Male") is the wrapper's `innerText` — never read `aria-label` first. The state lives on the hidden native `input[type=radio]` inside the wrapper: since ~2026-08-23 a synthetic click on the wrapper is inert, so click the inner input (`el.querySelector('input[type=radio]').click()`). After a change React REPLACES the wrapper nodes, so verify with a fresh query by question + option text, not via an attribute set on the old node. Reading `aria-label` first and clicking the wrapper produced 0 verified radio selections from 08-23 to 09-05.
- **Screener checkboxes (verified live 2026-09-13):** same widget family as the radios: `<p>statement*</p><fieldset aria-describedby="error-message-…">` containing a `div[role="checkbox"]` wrapper with a hidden native `input[type=checkbox]`, an empty `label[for]`, and the option text ("I certify and agree") in a `<p>` inside the wrapper. The hidden input is invisible to `fillForm` and the wrapper click is inert; `fillDialogCheckboxGroups` reads the statement from the fieldset's preceding sibling, checks consent-worded boxes through the inner input, verifies with a fresh query (the wrapper is re-rendered), and leaves SMS/marketing consent and unknown statements unchecked as guard refusals.
- **Text inputs:** interact via the ELEMENT HANDLE (`handle.fill()`/`handle.type()`), never a re-resolved `#id`/`[name=]` selector — the dialog UI reuses ids/names on hidden template nodes and typing lands elsewhere. The contact step's required "Location (city)" control is a typeahead `<input data-testid="typeahead-input" aria-autocomplete="list">` with NO `type` attribute and no label association (placeholder "Enter city or location"); selectors must include `input:not([type])`. Its suggestions render in a `[role="listbox"]` whose `aria-labelledby` is the input id; `typeValue`'s autocomplete handling picks one (verified live 2026-09-13).
