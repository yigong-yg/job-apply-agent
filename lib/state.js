...
function initSchema() {
  ...
  if (!runCols.includes('mode')) {
    // Pre-migration rows are overwhelmingly production runs; dry runs from
    // before the column existed stay labeled production, which only makes
    // cooldowns marginally more conservative.
...
function getRunStats(runId) {
  ...
  for (const row of rows) {
    if (!stats[row.platform]) {
      stats[row.platform] = { applied: 0, skipped: 0, errors: 0, dry_run: 0, already_applied: 0, captcha_blocked: 0 };
    }
