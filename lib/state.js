 ...
 * Return count of submitted applications today (Mountain Daylight Time = UTC-6).
...
function denverDayStartUTC(dateStr) {
  // Create a Date at midnight Denver time by parsing in that timezone
  const midnight = new Date(`${dateStr}T00:00:00`);
  // toLocaleString trick to get the offset: format in UTC and Denver, compare
  const utcStr = midnight.toLocaleString('en-US', { timeZone: 'UTC' });
  const denverStr = midnight.toLocaleString('en-US', { timeZone: 'America/Denver' });
  const utcMs = new Date(utcStr).getTime();
  const denverMs = new Date(denverStr).getTime();
  const offsetMs = utcMs - denverMs;
