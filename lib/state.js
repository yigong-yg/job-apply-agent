...
/**
 * Count submitted applications this ISO week (Mon-Sun).
 */
function getWeekSubmittedCount() {
  const d = getDb();
  // SQLite: strftime('%W', date) = ISO week number
  const row = d.prepare(
    "SELECT COUNT(*) as count FROM applications WHERE status = 'submitted' AND strftime('%W', appliedAt) = strftime('%W', 'now') AND strftime('%Y', appliedAt) = strftime('%Y', 'now')"
  ).get();
  return row.count;
}
