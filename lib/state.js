...
function initSchema() {
  ...
  if (!runCols.includes('mode')) {
    d.exec("ALTER TABLE runs ADD COLUMN mode TEXT DEFAULT 'production'");
  }

  // Legacy dry runs are identifiable from their outcome rows. Leaving the
  // defaulted production mode in place makes their errors and guarded skips
  // suppress real candidates through the failure cooldown and company cap.
  d.exec(`
    UPDATE runs
    SET mode = 'dry_run'
    WHERE id IN (
      SELECT DISTINCT runId FROM applications WHERE status = 'dry_run'
    )
  `);
}

// Rows written by dry runs must not feed production cooldowns or caps: a
// diagnostic dry run that honestly abandons a form would otherwise block the
// same job for the production run minutes later.
const PRODUCTION_ROWS_SQL = "runId IN (SELECT id FROM runs WHERE mode IS NULL OR mode != 'dry_run')";
...
function hasApplied(platform, jobId) {
  ...
  const row = d.prepare(
    `SELECT id FROM applications
     WHERE platform = ? AND jobId = ?
       AND (
         status = 'submitted'
         OR (
           status = 'already_applied'
           AND substr(skipReason, 1, 16) = 'already_applied_'
           AND skipReason != 'already_applied_db'
         )
       )`
  ).get(platform, jobId);
  return !!row;
...
// Terminal-failure predicate shared by the failure cooldown and the attempt
// cap: hard errors plus honest abandonments (guard-refused required fields,
// Top-Choice-gated forms). These fail the same way on every retry.
const TERMINAL_FAILURE_SQL = `(
      status IN ('error', 'submit_unconfirmed')
      OR (status = 'skipped' AND (skipReason LIKE 'guarded_required_field%' OR skipReason = 'top_choice_required'))
    )`;

/**
 * Failure cooldown (2026-07-28 review): a form that errored or was abandoned
 * on guard-refused fields fails identically on re-attempt, but errors were
 * invisible to every dedup layer — Kobie minted 7 "AI Engineer" jobIds in one
 * day and the agent burned a full form attempt on each. Matches the exact
 * jobId OR the normalized company+title (repost clones mint fresh jobIds).
 *
 * @returns {boolean}
 */
function hasRecentFailure({ platform, jobId, company, jobTitle, days = 14 }) {
  const cutoff = new Date(Date.now() - days * 86400000).toISOString();
  const rows = getDb().prepare(`
    SELECT jobId, company, jobTitle FROM applications
    WHERE platform = ? AND appliedAt >= ? AND ${TERMINAL_FAILURE_SQL}
      AND ${PRODUCTION_ROWS_SQL}
  `).all(platform, cutoff);

  if (jobId && rows.some((r) => r.jobId === jobId)) return true;
...
/**
 * Attempts (submissions + terminal failures) to a normalized company in the
 * last `days`. Backs the per-company cap: a company whose forms always fail
 * still consumes form time on every clone, so failed attempts count against
 * its budget too.
 *
 * @returns {number}
 */
function getCompanyRecentAttemptCount({ platform, company, days = 30 }) {
  const normCompany = normalizeCompany(company);
  if (!normCompany) return 0;
  ...
  const rows = getDb().prepare(`
    SELECT company FROM applications
    WHERE platform = ? AND appliedAt >= ? AND company IS NOT NULL
      AND (status = 'submitted' OR ${TERMINAL_FAILURE_SQL})
      AND ${PRODUCTION_ROWS_SQL}
  `).all(platform, cutoff);
...
function recordApplication({ platform, jobId, jobTitle, company, jobUrl, status, errorMessage, skipReason, runId, source }) {
  ...
  if (status === 'already_applied') {
    const expectedEvidence = `already_applied_${platform}`;
    if (skipReason !== 'already_applied_db' && skipReason !== expectedEvidence) {
      throw new Error(
        `already_applied requires skipReason "already_applied_db" or "${expectedEvidence}"`
      );
    }
  }
 ...
 */
function createRun({ mode = 'production' } = {}) {
  const d = getDb();
  const runId = uuidv4();
  const now = new Date().toISOString();
  ...
  d.prepare(`
    INSERT INTO runs (id, sessionId, startedAt, mode) VALUES (?, ?, ?, ?)
  `).run(runId, sessionId, now, mode === 'dry_run' ? 'dry_run' : 'production');
...
function recordFillAudit({ platform, jobId, runId, fieldLabel, fieldType, inputType, fillSource, answer, confidence }) {
  ...
  d.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(platform, jobId || null, runId || 'unattributed', fieldLabel, fieldType || null, inputType || null, fillSource, answer || null, confidence || null, now);
}
...
function getRunStats(runId) {
  ...
  for (const row of rows) {
    ...
    else if (row.status === 'captcha_blocked') stats[row.platform].captcha_blocked += row.count;
    else if (row.status === 'submit_unconfirmed') {
      stats[row.platform].submit_unconfirmed += row.count;
      stats[row.platform].errors += row.count;
    }
  }
  return stats;
}
...
/**
 * Count external Submit clicks that consume today's safety budget. An
 * unconfirmed click may still have submitted successfully, so it must count
 * conservatively even though it is not proof for hasApplied().
 */
function getTodaySubmissionBudgetCount() {
  const d = getDb();
  const denverToday = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Denver' });
  const row = d.prepare(
    "SELECT COUNT(*) as count FROM applications WHERE status IN ('submitted', 'submit_unconfirmed') AND appliedAt >= ? AND appliedAt < ?"
  ).get(denverDayStartUTC(denverToday), denverDayStartUTC(nextDay(denverToday)));
  return row.count;
}
...
function getFailReasons(runId) {
  ...
  return d.prepare(`
    ...
    FROM applications
    WHERE runId = ? AND status IN ('error', 'submit_unconfirmed', 'skipped') AND (skipReason IS NULL OR skipReason NOT IN ('already_applied_db', 'already_applied_linkedin', 'no_easy_apply_button', 'promoted'))
