...
function getRunExitCode({
  ...
  if (captchaBlocked) return 1;
  if (!dryRun && unconfirmedSubmissions > 0) return 1;
...
function clampToDailyBudget({ perRunMax = 0, dailyCap = 0, submittedToday = 0 } = {}) {
  const numericCap = Number(dailyCap);
  const numericSubmitted = Number(submittedToday);
  const numericPerRun = Number(perRunMax);
  if (!Number.isFinite(numericCap)) return 0;
  if (!Number.isFinite(numericSubmitted) || !Number.isFinite(numericPerRun)) return 0;
  const remaining = Math.max(
    0,
    Math.floor(numericCap) - Math.max(0, numericSubmitted)
  );
  if (numericPerRun < 0) return 0;
  if (numericPerRun === 0) return remaining;
  return Math.min(Math.floor(numericPerRun), remaining);
}
