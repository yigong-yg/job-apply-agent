...
// The LLM's honest-refusal sentinel is an instruction to leave the field
// empty, never a value ("Address Line 1" → "CANNOT_ANSWER" reached a fill).
const REFUSAL_RE = /^\s*cannot_answer\b/i;
...
function validateAnswer(answer, field = {}) {
  ...
  if (REFUSAL_RE.test(value)) return { ok: false, reason: 'refusal_sentinel' };
...
// [source, flags] pairs for in-page (shadow DOM) value rejection, where
// page.evaluate() cannot import this module.
const VALIDATOR_PATTERN_SOURCES = [
  [PLACEHOLDER_RE.source, ''],
  [MARKDOWN_RE.source, ''],
  [META_ANSWER_RE.source, 'i'],
  [GENERIC_FALLBACK_RE.source, 'i'],
  [REFUSAL_RE.source, 'i'],
];

module.exports = { validateAnswer, VALIDATOR_PATTERN_SOURCES };
