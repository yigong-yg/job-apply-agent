...
const STATUS_MAP = {
  ...
  submit_unconfirmed: { icon: '⚠️', label: 'UNVERIFIED', color: 15105570 },
...
function normalizeReason(status, skipReason, errorMessage) {
  ...
  if (status === 'submit_unconfirmed') return 'submit_unconfirmed';
  if (skipReason) {
 ...
 */
async function sendSessionSummary({ sessionId, durationMin, scanned, applied, readyToSubmit = 0, alreadyApplied = 0, skipped, failed, dsCalls, topFail, weekApplied, weekTarget, dbTotal, dryRun }, logger) {
  if (!DISCORD_WEBHOOK_URL) return;

  const mode = getSessionMode(dryRun);
  const successful = dryRun ? readyToSubmit : applied;
  const successRate = (successful + failed) > 0
    ? Math.round(100 * successful / (successful + failed))
    : 100;
  ...
  const fields = [
    ...
    { name: dryRun ? 'ready_to_submit' : 'applied', value: String(successful), inline: true },
    { name: 'already_applied', value: String(alreadyApplied), inline: true },
  ...
  const textContent = [
    ...
    dryRun
      ? `duration=${durationMin}min scanned=${scanned} ready_to_submit=${readyToSubmit} already_applied=${alreadyApplied} skipped=${skipped} failed=${failed}`
      : `duration=${durationMin}min scanned=${scanned} applied=${applied} already_applied=${alreadyApplied} skipped=${skipped} failed=${failed}`,
 ...
 */
function buildMeowfisMessage({ dryRun = false, sessionId, applied = 0, readyToSubmit = 0, scanned = 0 } = {}) {
  if (dryRun) {
    return `Dry run #${sessionId} complete. No dashboard needed. scanned=${scanned} ready_to_submit=${readyToSubmit}`;
  }
