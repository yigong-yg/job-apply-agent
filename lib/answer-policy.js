...
const CLASS_RULES = [
  ...
  { questionClass: 'work_authorization', re: /authorized to work|legally authorized|eligible to work|legal right to work|right to work in|work authorization|lawfully work|\b(un)?able to (lawfully|legally) work\b|\b(un)?able to work in the (us|usa|u s a?|united states)\b/ },
];
...
// Some screeners name no jurisdiction at all and defer to the posting:
// "authorized to work in the country where this position is located",
// "the country you are applying to". The posting's country is known to the
// caller (card location / search scope) and is passed in as `jobCountry`;
// without it these questions stay unanswerable.
const JOB_COUNTRY_PLACEHOLDER_RE = /\b(?:the )?country (?:where|in which) (?:this|the) (?:position|job|role) is (?:located|based)\b|\bcountry (?:you are|you re) applying (?:to|for|in)\b|\bthe country of (?:employment|the position|the job)\b|\bthis country\b/;

function namedJurisdictions(text, rawText = text, jobCountry = null) {
  ...
  if (jobCountry && JOB_COUNTRY_PLACEHOLDER_RE.test(normalized)) {
    named.add(String(jobCountry));
  }
  return named;
}

function authorizationCoversJurisdiction(label, workAuthorization, rawLabel = label, jobCountry = null) {
  const asked = namedJurisdictions(label, rawLabel, jobCountry);
...
function resolveFromConfig(questionClass, label, config, defaultAnswers, rawLabel = label, jobCountry = null) {
  ...
  switch (questionClass) {
    case 'work_authorization':
      ...
      if (user.workAuthorization) {
        const named = namedJurisdictions(label, rawLabel, jobCountry);
        ...
        if (authorizationCoversJurisdiction(label, user.workAuthorization, rawLabel, jobCountry)) {
          return { answer: 'Yes', source: 'config:user.workAuthorization' };
        }
      }
      return null;
    ...
    case 'sponsorship':
      if (typeof user.requiresSponsorship === 'boolean') {
        ...
        if ((named.size > 0 || hasJurisdictionTail) &&
            !authorizationCoversJurisdiction(label, user.workAuthorization, rawLabel, jobCountry)) return null;
    ...
    case 'citizenship': {
      ...
      if (!authorizationCoversJurisdiction(label, authorizationRaw, rawLabel, jobCountry)) return null;
 ...
 * @param {{config: object, defaultAnswers: object, jobCountry?: string|null}} ctx
 *   `jobCountry` is the posting's country as an answer-policy jurisdiction
 *   name ('us', 'canada', ...); it grounds "the country where this position
 *   is located" phrasings and is otherwise ignored.
...
function guardAnswer(rawLabel, ctx = {}) {
  ...
  const jobCountry = ctx.jobCountry ? normalize(ctx.jobCountry) : null;
  const resolved = resolveFromConfig(questionClass, label, ctx.config, ctx.defaultAnswers, rawLabel, jobCountry);
...
module.exports = { classifyQuestion, guardAnswer, normalize, namedJurisdictions, GUARDED_PATTERN_SOURCES };
