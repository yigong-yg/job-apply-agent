...
function inferByRules(normalLabel, rawLabel, fieldType, config) {
  ...
  const rules = [
    ...
    { rule: 'commute', test: () => normalLabel.includes('commut'), answer: 'Yes' },
...
async function handleAutocompleteDropdown(page, input, typedValue) {
  ...
  // Try exact substring match first (e.g. "Springfield" in "Springfield, IL, United States")
...
async function fillForm(page, defaultAnswers, config, logger, platform = 'unknown', jobId = null, options = {}) {
  ...
  for (const input of textInputs) {
    ...
    if (guarded) {
      ...
      logger.debug({ platform, jobId, field: rawLabel, answer: guardValue }, 'Filled text field (Tier 0 guard config)');
      continue;
    }
    ...
    if (answer !== null) {
      ...
      if (!v.ok) {
        logger.debug({ platform, jobId, field: rawLabel, rejectedAnswer: String(answer).substring(0, 60), reason: v.reason }, 'Tier 1 output guard rejected answer');
        answer = null; // Fall through to Tier 2
      }
    }
  ...
  for (const select of selects) {
    ...
    if (guarded) {
      if (guarded.blocked) continue;
      const matched = nonPlaceholderOpts.length > 0 ? matchDropdownOption(guarded.answer, nonPlaceholderOpts) : null;
      if (matched) {
    ...
    try {
      ...
      if (isSelectEl) {
        ...
        if (bestMatch.rating >= 0.4) {
          ...
          if (matchedText) {
            ...
            await sleep(200, 400);
            filledCount++;
            auditFill(platform, jobId, runId, rawLabel, 'custom-dropdown', null, answerSource, matchedText, 'fuzzy');
      ...
      for (const opt of dropdownOptions) {
        ...
        if (optText.toLowerCase().includes(answer.toLowerCase()) || answer.toLowerCase().includes(optText.toLowerCase())) {
          ...
          filledCount++;
          matched = true;
          auditFill(platform, jobId, runId, rawLabel, 'custom-dropdown', null, answerSource, optText, 'substring');
      ...
      if (!matched && dropdownOptions.length > 0) {
        ...
        if (bestMatch.rating >= 0.4) {
          ...
          if (matchIdx >= 0) {
            ...
            filledCount++;
            matched = true;
            auditFill(platform, jobId, runId, rawLabel, 'custom-dropdown', null, answerSource, optTexts[matchIdx], 'fuzzy');
  ...
  for (const [groupName, groupRadios] of radioGroups) {
    ...
    if (guarded) {
      if (guarded.blocked) continue;
      const targetLower = guarded.answer.toLowerCase();
      const matched = radioOptions.find((o) => {
        const l = o.label.toLowerCase();
        return l && (l === targetLower || l.includes(targetLower) || (l.length >= 3 && targetLower.includes(l)));
      });
      if (matched) {
...
async function retryInvalidFields(page, defaultAnswers, config, logger, platform, jobId, options = {}) {
  ...
  for (const el of uniqueInvalid) {
    ...
    if (answer !== null) {
      ...
      if (!v.ok) {
        logger.debug({ platform, jobId, field: rawLabel, rejectedAnswer: String(answer).substring(0, 60), reason: v.reason }, 'Retry output guard rejected answer');
        answer = null;
      }
    }
