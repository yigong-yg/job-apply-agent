...
const NUMERIC_LABEL_INDICATORS = [
  'how many',
  'number of',
  'years of',
  'months of',
...
function isNumericQuestion(normalLabel, inputType = '') {
  return inputType === 'number' ||
    NUMERIC_LABEL_INDICATORS.some((indicator) => normalLabel.includes(indicator));
}

function coerceNumericAnswer(answer) {
...
function prepareTextAnswer(answer, normalLabel, inputType = '') {
  if (answer === null || answer === undefined) return null;
  let value = String(answer).trim();
  if (!value) return null;

  if (isNumericQuestion(normalLabel, inputType)) {
    value = coerceNumericAnswer(value);
...
function inferByRules(normalLabel, rawLabel, fieldType, config) {
  ...
  const fullName = firstNonEmpty(`${user.firstName || ''} ${user.lastName || ''}`);
  const graduationYear = firstNonEmpty(user.graduationYear, user.educationGraduationYear, user.degreeYear);
  const githubUrl = firstNonEmpty(user.githubUrl, user.github, user.gitHubUrl);
  const website = firstNonEmpty(user.website, user.portfolioUrl, user.personalWebsite);
  const currentEmployer = firstNonEmpty(user.currentEmployer, user.mostRecentEmployer, user.employer, 'N/A');
  const defaultPercentage = firstNonEmpty(user.defaultPercentageAnswer, user.defaultPercentage, '50');
  ...
  const rules = [
    ...
    { rule: 'preferred_name', test: () => normalLabel === 'preferred name', answer: firstNonEmpty(user.preferredName, fullName, user.firstName) },
    { rule: 'first_name', test: () => normalLabel === 'first name' || normalLabel === 'legal first name' || normalLabel === 'given name', answer: user.firstName },
    { rule: 'middle_name', test: () => normalLabel === 'middle name' || normalLabel === 'middle initial', answer: firstNonEmpty(user.middleName, 'N/A') },
    ...
    { rule: 'graduation_year', test: () => normalLabel.includes('graduation year') || (normalLabel.includes('year') && normalLabel.includes('degree')), answer: graduationYear },
    { rule: 'percentage_numeric', test: () => normalLabel.includes('percent') || normalLabel.includes('percentage') || normalLabel.includes('development time'), answer: defaultPercentage },
    ...
    { rule: 'salary', test: () => normalLabel.includes('salary') || normalLabel.includes('compensation') || normalLabel.includes('pay rate') || normalLabel.includes('desired pay') || normalLabel.includes('base pay') || normalLabel.includes('pay expectation'), answer: user.desiredSalary },
    ...
    { rule: 'github_url', test: () => normalLabel.includes('github'), answer: githubUrl },
    { rule: 'website_url', test: () => normalLabel === 'website' || normalLabel.includes('portfolio') || normalLabel.includes('personal website'), answer: website },
    { rule: 'current_employer', test: () => normalLabel.includes('current employer') || normalLabel.includes('most recent employer'), answer: currentEmployer },
    ...
    { rule: 'hear_about', test: () => normalLabel.includes('hear about') || normalLabel.includes('how did you hear') || hasWholePhrase(normalLabel, 'referred by') || hasWholePhrase(normalLabel, 'referral source'), answer: 'Job Board' },
  ];
...
async function fillForm(page, defaultAnswers, config, logger, platform = 'unknown', jobId = null, options = {}) {
  ...
  for (const input of textInputs) {
    ...
    if (answer !== null && prepareTextAnswer(answer, normalLabel, inputType) === null) {
    ...
    if (answer !== null) {
      const fillValue = prepareTextAnswer(answer, normalLabel, inputType);
    ...
    if (ruleResult) {
      const fillValue = prepareTextAnswer(ruleResult.answer, normalLabel, inputType);
      if (fillValue === null) {
        logger.debug({ platform, jobId, field: rawLabel, rejectedAnswer: ruleResult.answer }, 'Tier 2 format guard: rejected non-numeric answer for numeric field');
        continue;
      }
      await typeValue(page, input, fillValue, config);
      filledCount++;
    ...
    if (canUseLlm) {
      ...
      if (llmAnswer && llmAnswer !== FALLBACK_ANSWER) {
        const fillValue = prepareTextAnswer(llmAnswer, normalLabel, inputType);
        if (fillValue === null) {
          logger.debug({ platform, jobId, field: rawLabel, rejectedAnswer: llmAnswer }, 'Tier 3 format guard: rejected non-numeric answer for numeric field');
          continue;
        }
        await typeValue(page, input, fillValue, config);
        filledCount++;
        auditFill(platform, jobId, runId, rawLabel, tagName, inputType, `llm:${mode}`, fillValue, 'llm');
        logger.info({ platform, jobId, field: rawLabel.substring(0, 80), source: `llm:${mode}`, answerLength: fillValue.length }, 'Filled field via LLM (Tier 3)');
        continue;
      }
  ...
  for (const el of uniqueInvalid) {
    ...
    if (answer !== null) {
      if (tagName === 'select') {
        ...
        const fillValue = prepareTextAnswer(answer, normalLabel, inputType);
        if (fillValue === null) {
          logger.debug({ platform, jobId, field: rawLabel, rejectedAnswer: answer }, 'Retry format guard: rejected non-numeric answer for numeric field');
          continue;
        }
        if (hadWrongValue) {
