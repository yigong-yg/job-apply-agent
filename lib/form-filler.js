...
function deriveGraduationYear(user) {
  if (user.graduationYear) return String(user.graduationYear);

  const currentYear = new Date().getUTCFullYear();
  const years = parseInt(user.yearsOfExperience || '', 10);
  if (!Number.isNaN(years) && years >= 0 && years <= 50) {
    return String(currentYear - Math.max(years, 1));
  }
...
function inferByRules(normalLabel, rawLabel, fieldType, config) {
  ...
  const rules = [
    ...
    { rule: 'graduation_year', test: () => (normalLabel.includes('graduation') && normalLabel.includes('year')) || normalLabel.includes('year of graduation') || (normalLabel.includes('completed degree') && normalLabel.includes('year')), answer: deriveGraduationYear(user) },
    { rule: 'percentage_numeric', test: () => normalLabel.includes('percentage') || hasWholePhrase(normalLabel, 'percent') || rawLabelLower.includes('%'), answer: '50.0' },
