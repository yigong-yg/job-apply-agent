...
// Country selects use display names; config stores a code or a name.
const COUNTRY_DISPLAY_NAMES = {
  us: 'United States', usa: 'United States', 'united states': 'United States',
  'united states of america': 'United States', ca: 'Canada', canada: 'Canada',
  uk: 'United Kingdom', gb: 'United Kingdom', 'united kingdom': 'United Kingdom',
  au: 'Australia', australia: 'Australia', in: 'India', india: 'India',
  mx: 'Mexico', mexico: 'Mexico', de: 'Germany', germany: 'Germany', fr: 'France', france: 'France',
};
const COUNTRY_DIALING_CODES = {
  'United States': '+1', Canada: '+1', 'United Kingdom': '+44', Australia: '+61',
  India: '+91', Mexico: '+52', Germany: '+49', France: '+33',
};
function countryDisplayName(value) {
  const key = String(value || '').trim().toLowerCase();
  if (!key) return null;
  return COUNTRY_DISPLAY_NAMES[key] || String(value).trim();
}
// LinkedIn's phone country code options read "United States (+1)".
function phoneCountryCodeAnswer(value) {
  const name = countryDisplayName(value);
  if (!name) return null;
  const code = COUNTRY_DIALING_CODES[name];
  return code ? `${name} (${code})` : name;
}
const COUNTRY_LABEL_RE = /^(?:(?:current |home |residence )?country(?: of residence| region| name)?|(?:which|what) country (?:are you|do you) (?:currently )?(?:in|located in|live in|reside in|based in))$/;
const PHONE_COUNTRY_CODE_LABEL_RE = /^(?:(?:phone|mobile|telephone) )?country code$|^phone country$|\bdial(?:ing)? code\b/;
...
function inferByRules(normalLabel, rawLabel, fieldType, config) {
  ...
  const rules = [
    ...
    // selects; with no rule both stayed unselected and the step bounced
    // (3 error cycles, 2026-09-15..17). Phone code first: it also says
    // "country". Labels are matched whole so prose mentioning a country is
    // never answered with the configured one.
    { rule: 'phone_country_code', test: () => PHONE_COUNTRY_CODE_LABEL_RE.test(normalLabel), answer: phoneCountryCodeAnswer(user.country) },
    { rule: 'country', test: () => COUNTRY_LABEL_RE.test(normalLabel), answer: countryDisplayName(user.country) },
...
async function handleAutocompleteDropdown(page, input, typedValue, dropdownContext) {
  // Declared typeaheads (aria-autocomplete="list", combobox, aria-controls)
  // fetch their suggestions asynchronously, and LinkedIn's location list
  // regularly takes longer than one short wait (7Th Sky Tech, 2026-09-17:
  // the raw text stayed in the field and the contact step bounced). Poll
  // them for a few seconds; plain inputs keep the single brief wait.
  const isDeclaredTypeahead = await input.evaluate((el) =>
    el.getAttribute('aria-autocomplete') === 'list' || el.getAttribute('role') === 'combobox' ||
    !!el.getAttribute('aria-controls') || !!el.getAttribute('aria-owns')
  ).catch(() => false);
  const deadline = Date.now() + (isDeclaredTypeahead ? 4000 : 0);
  let visibleOptions = [];
  do {
    await sleep(400, 800);
    visibleOptions = await collectOwnedOrOpenedOptions(page, dropdownContext);
  } while (visibleOptions.length === 0 && Date.now() < deadline);
  ...
  // A suggestion counts as applied when the field no longer holds the raw
  // typed text or the list has closed; a typeahead that keeps the raw text
  // fails validation ("This field is required") on the next step.
  const selectionApplied = async () => {
    const valueChanged = await input.evaluate(
      (el, typed) => (el.value || '').trim() !== String(typed).trim(), typedValue
    ).catch(() => false);
    if (valueChanged) return true;
    const stillOpen = await collectOwnedOrOpenedOptions(page, dropdownContext);
    return stillOpen.length === 0;
  };

  let clicked = false;
  try {
    // Bounded: the dialog overlay can make a covered option unclickable, and
    // the default action timeout would stall the step for 30 s.
    await visibleOptions[bestIdx].click({ force: true, timeout: 5000 });
    clicked = true;
    await sleep(200, 400);
  } catch (_) {}
  if (!clicked || (isDeclaredTypeahead && !(await selectionApplied()))) {
    // The keyboard path selects the highlighted suggestion in most typeaheads.
    try {
...
function runGuard(rawLabel, fieldType, config, answers, ctx) {
  const { platform, jobId, runId, logger, unfilledFields, jobCountry } = ctx;
  const guard = guardAnswer(rawLabel, { config, defaultAnswers: answers, jobCountry: jobCountry || null });
 ...
 * @param {object} [options.jobContext] - { jobTitle, company, jobDescription, jobCountry }
...
async function fillForm(page, defaultAnswers, config, logger, platform = 'unknown', jobId = null, options = {}) {
  ...
  // Posting country (answer-policy jurisdiction name) that grounds
  // "the country where this position is located" screeners.
  const jobCountry = options.jobContext?.jobCountry || null;
  let filledCount = 0;
  ...
  // ─────────────────────────────────────────────────────────────────────
  // `input:not([type])` is a text input by definition. LinkedIn's required
  // contact-step "Location (city)" typeahead (data-testid="typeahead-input",
  // aria-autocomplete="list") carries no type attribute; without this clause
  // it was never filled and the step bounced on "This field is required"
  // (11 jobs recorded as errors 2026-09-06..13).
  const textInputs = await queryAllInScope(page, 'input:not([type]), input[type="text"], input[type="number"], input[type="tel"], input[type="email"], input[type="url"], textarea', options);
  ...
  for (const input of textInputs) {
    ...
    const guarded = runGuard(rawLabel, guardFieldType, config, answers, { platform, jobId, runId, logger, unfilledFields, jobCountry });
    if (guarded) {
  ...
  for (const select of selects) {
    ...
    // ── Tier 0: never-auto-answer guard (spec R8) ──
    const guarded = runGuard(rawLabel, 'select', config, answers, { platform, jobId, runId, logger, unfilledFields, jobCountry });
    if (guarded) {
      if (guarded.blocked) continue;
      ...
      if (matched) {
        ...
        if (ok) {
          ...
          logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(matched.text).length, source: guarded.source }, 'Selected dropdown (Tier 0 guard config)');
          continue;
        }
      }
    ...
    if (answer !== null && nonPlaceholderOpts.length > 0) {
      ...
      if (matched) {
        ...
        if (ok) {
          ...
          logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(matched.text).length, source: 'defaultAnswers' }, 'Selected dropdown (Tier 1)');
          continue;
        }
      }
    ...
    if (ruleResult && nonPlaceholderOpts.length > 0) {
      ...
      if (matched) {
        ...
        if (ok) {
          ...
          logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(matched.text).length, rule: ruleResult.rule }, 'Selected dropdown (Tier 2)');
          continue;
        }
      }
    }
      ...
      if (polarityAnswer) {
        ...
        if (matched) {
          ...
          if (ok) {
            ...
            logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(matched.text).length, rule }, 'Selected yes/no dropdown (polarity)');
            continue;
          }
        }
      }
    ...
    if (nonPlaceholderOpts.length > 0 && LOW_RISK_FIRST_OPTION.test(normalLabel)) {
      ...
      if (ok) {
        ...
        logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(firstReal.text).length, source: 'safe_default' }, 'Selected first option (safe_default, low-risk)');
      }
    } else if (rawLabel) {
  ...
  for (const trigger of customDropdowns) {
    ...
    const guarded = runGuard(rawLabel, 'custom-dropdown', config, answers, { platform, jobId, runId, logger, unfilledFields, jobCountry });
    if (guarded) {
      if (guarded.blocked) continue;
    ...
    try {
      ...
      if (isSelectEl) {
        ...
        if (matchedText) {
          ...
          logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(matchedText).length, source: answerSource }, 'Selected custom dropdown (native select)');
          continue;
        }
      }
      ...
      if (matched) {
        ...
        logger.debug({ platform, jobId, field: rawLabel, selectedLength: String(optTexts[matchIdx]).length, source: answerSource }, 'Selected custom dropdown option');
      }
  ...
  for (const [groupName, groupRadios] of radioGroups) {
    ...
    // ── Tier 0: never-auto-answer guard (spec R8) ──
    const guarded = runGuard(questionLabel || groupName, 'radio', config, answers, { platform, jobId, runId, logger, unfilledFields, jobCountry });
    if (guarded) {
      if (guarded.blocked) continue;
      ...
      if (matched) {
        ...
        if (selected) {
          ...
          logger.debug({ platform, jobId, group: groupName, selectedLength: String(matched.label).length, source: guarded.source }, 'Selected radio (Tier 0 guard config)');
        } else {
    ...
    if (answer !== null) {
      ...
      if (matched && bestMatch.rating >= 0.4) {
        ...
        if (selected) {
          ...
          logger.debug({ platform, jobId, group: groupName, selectedLength: String(matched.label).length, source: 'defaultAnswers' }, 'Selected radio (Tier 1)');
        } else {
    ...
    if (ruleResult) {
      ...
      if (matched && bestMatch.rating >= 0.4) {
        ...
        if (selected) {
          ...
          logger.debug({ platform, jobId, group: groupName, selectedLength: String(matched.label).length, rule: ruleResult.rule }, 'Selected radio (Tier 2)');
          continue;
        }
      }
    }
...
async function retryInvalidFields(page, defaultAnswers, config, logger, platform, jobId, options = {}) {
  ...
  for (const el of uniqueInvalid) {
    ...
    logger.debug({ platform, jobId, field: rawLabel, currentValueLength: currentValue.length, errorMessage: errorMessage.substring(0, 100), hadWrongValue }, 'Retry analyzing invalid field');
    ...
    const guard = guardAnswer(rawLabel, {
      config, defaultAnswers: answers, jobCountry: options.jobContext?.jobCountry || null,
    });
